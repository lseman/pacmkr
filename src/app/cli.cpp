...
void set_bool(Cli& cli, const std::string& flag, bool val) {
    ...
    else if (flag == "downloadonly")     cli.sync_download = val;
    ...
    else if (flag == "query-file")        cli.query_file = val;
    else if (flag == "query-changelog")   cli.query_changelog = val;
...
constexpr OptDef opts[] = {
    ...
    {"--downloadonly", "-w", false},
    {"--debug", nullptr, false},
    {"--color", nullptr, false},
    {"--nocolor", nullptr, false},
    {"--all-deps", nullptr, false},
    {"--asdeps", nullptr, false},
    {"--asexplicit", nullptr, false},
...
Cli parse(int argc, char* argv[]) {
    ...
    while (i < static_cast<size_t>(argc)) {
        ...
        // Pacman operation selectors must win over makepkg-compatible aliases
        // such as -D/--dir and the repackage option.
        if (sarg == "-D" || sarg == "--database") { cli.operation = Cli::Op::Database; ++i; continue; }
        if (sarg == "-F" || sarg == "--files") { cli.operation = Cli::Op::Files; ++i; continue; }
        if (sarg == "-Q" || sarg == "--query") { cli.operation = Cli::Op::Query; ++i; continue; }
        if (sarg == "-R" || sarg == "--remove") { cli.operation = Cli::Op::Remove; ++i; continue; }
        if (sarg == "-S") { cli.operation = Cli::Op::Sync; ++i; continue; }
        if (sarg == "-T" || sarg == "--deptest") { cli.operation = Cli::Op::Deptest; ++i; continue; }
        if (sarg == "-U" || sarg == "--upgrade") { cli.operation = Cli::Op::Upgrade; ++i; continue; }
        ...
        for (auto& def : opts) {
            ...
            if (check_arg == def.long_flag || (def.short_flag && sarg == def.short_flag)) {
                if (def.has_value) {
                    ...
                    else if (lf == "ignore") cli.ignore.push_back(value);
                    ...
                    else if (lf == "flags") cli.show_flags = true;
                    else if (lf == "list-foreign") {
                        cli.operation = Cli::Op::Query;
                        cli.query_mirrors = true;
                    } else if (lf == "asdeps") cli.database_remove = true;
                    else if (lf == "asexplicit") cli.database_add = true;
...
void print_help() {
    const char* help_text =
        ...
        "Common pacman-compatible operation forms include Q, R, S, and U.\n"
        ...
        "  -Q, --query       Query local package DB (-Qi, -Qii, -Ql, -Qe, -Qu, -Qs,\n"
        "                    -Qp FILE, -Qc PKG, etc.)\n"
        "  -R, --remove      Remove packages (-Rs, -Rc, -Rd, -Rns)\n"
        "  -S, --sync        Sync/install packages (-Sy, -Su, -Ss, -Si, -Sii, -Sl,\n"
        "                    -Sw to download without installing,\n"
        "                    -Sc / -Scc to clean the package cache, etc.)\n"
        "  -T, --deptest     Test dependency resolution\n"
        "  -U, --upgrade     Upgrade from a local file or http(s) URL\n"
        ...
        "      --flags               Print the compiler and flags used to build packages\n";
    std::cout << help_text;
}
